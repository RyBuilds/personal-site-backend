...
export const refreshAccessToken = async (http: Axios, config: SpotifyConfig) => {
  console.log('Refreshing access token')
  console.log('----')

  const url = 'https://accounts.spotify.com/api/token'
  const auth = Buffer.from(`${config.clientId}:${config.secret}`, 'binary').toString('base64');
  const form = {grant_type:'refresh_token', refresh_token: config.refreshToken}
  const headers = {
...
export const getAccessToken = async (http: Axios, config: SpotifyConfig) => {
  return spotifyAccessToken ? spotifyAccessToken : await refreshAccessToken(http, config)
}
